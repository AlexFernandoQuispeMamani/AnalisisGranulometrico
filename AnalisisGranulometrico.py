...
if st.session_state.page == "input":
    ...
    with col1:
        ...
        peso_total_input = st.number_input("Peso total (g)", min_value=0.01, step=0.01)
        ...
        # Botón CALCULAR: guarda datos y cambia a la ventana de resultados
        if st.button("CALCULAR", type="primary"):
            st.session_state.edited_df = edited_df_local
            st.session_state.peso_total = peso_total_input
...
# --- PÁGINA: Resultados ---
    ...
    # Mantener títulos arriba (no modificados). Añadimos botón para volver.
    volver_col, info_col = st.columns([1, 5])
    with volver_col:
        if st.button("← Volver a entrada"):
            st.session_state.page = "input"
            st.session_state["calculado"] = False

    # Recuperar datos guardados
    peso_total = st.session_state.peso_total
    edited_df = st.session_state.edited_df

    # Elegir gráfico (ahora con opción COMBINADO)
    grafico_seleccionado = st.radio(
        "Seleccione el gráfico a mostrar:",
        ["DISTRIBUCIÓN POR CLASES (%Peso)", "%ACUMULADO PASANTE", "%ACUMULADO RETENIDO", "COMBINADO (3 curvas)"],
        horizontal=True
    )

    # Sólo procesar si hay datos guardados
    if st.session_state["calculado"]:
        # --- MANTENGO TU LÓGICA DE CÁLCULO EXACTA ---
        df = edited_df.dropna()
        ...
        if not df.empty and peso_total > 0:
            ...
            st.dataframe(df[["Tamaño (μm)", "Peso retenido (g)", "%Peso", "%R(d)", "%F(d)"]], use_container_width=True)
            ...
            elif grafico_seleccionado == "COMBINADO (3 curvas)":
                # Graficar las tres curvas en la misma figura
                # Aseguramos que existan las columnas necesarias
                ax.plot(df_plot["Tamaño (μm)"], df_plot["%Peso"], marker='x', linestyle='-', label='%Peso', linewidth=1)
                # Acumulado pasante (%F(d))
                if "%F(d)" in df_plot.columns:
                    ax.plot(df_plot["Tamaño (μm)"], df_plot["%F(d)"], marker='o', linestyle='--', label='%Pasante', linewidth=1)
                # Acumulado retenido (%R(d))
                if "%R(d)" in df_plot.columns:
                    ax.plot(df_plot["Tamaño (μm)"], df_plot["%R(d)"], marker='s', linestyle=':', label='%Retenido', linewidth=1)

                ax.set_ylim(0, 100)
                ax.set_ylabel("%")
                ax.set_title("COMBINADO: %Peso | %Pasante | %Retenido")
                ax.legend()

            st.pyplot(fig)

            # --- (A partir de aquí, mantengo exactamente tu cálculo de d_XX, Folk & Ward, interpretaciones, etc.) ---
            ...
            # Estadística Folk & Ward (igual que en tu código)
